...
1. **Product Brand** (field ID from `jira-config.md > Static Field Values`) — if populated, use as domain label (e.g., "the LMS product", "the credentials product")
...
Selected:
- SpeedGrader Phase 2 (the LMS product, Done) — Enabler: adds moderated grading, directly enables multi-evaluator routing this idea requires. *Selected because: specific integration point, Done status (proven investment).*
- Enhanced Rubrics Phase 3 (the LMS product, Done) — Enabler: rubric infrastructure quality is foundational to any qualitative evaluation workflow on SpeedGrader. *Selected because: specifically load-bearing for a grading idea — not general infrastructure.*
- DocViewer rotated document annotation (the LMS product, GTM) — Enabler: restores annotation on photographed handwritten work, exactly the artifact type authentic assessment produces. *Selected because: specific to this idea's multi-modal grading surface, shipping imminently.*
- PDB Outcome & Rubrics integration (the credentials product, Opportunity Identification) — Convergence: the credentials product building rubric-based credentialing — different angle on the same need for trustworthy rubric evaluation of qualitative work. *Selected because: different domain, same underlying need.*

Excluded:
- Enhanced Rubrics pre-GA Tasks (the LMS product, Done) — duplicative with Enhanced Rubrics Phase 3, null description, low incremental value.
- Responsive SpeedGrader (the LMS product, Done) — general UI infrastructure (responsive design, accessibility). Benefits every SpeedGrader feature, not specific to qualitative evaluation.

Convergence group:
> Rubric evaluation quality for downstream decisions — SpeedGrader Phase 2 (the LMS product), Enhanced Rubrics Phase 3 (the LMS product), PDB Outcome & Rubrics (the credentials product). the LMS product is investing in rubric grading infrastructure while the credentials product is building rubric-based credentialing — both need rubric evaluation to work well for non-traditional assessment, creating shared organizational interest that this idea can leverage.
