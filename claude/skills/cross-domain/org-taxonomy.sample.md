# Organizational Structural Reference

Reference file for cross-domain discovery. Describes your JPD project's organizational taxonomy so the LLM can evaluate functional overlap from summary + structural metadata.

Copy this file to the filename referenced in your /cross-domain skill (default: `pm2025-structural-reference.md`) and fill in your organization's structure.

**Update cadence:** When org changes are noticed (team renames, new squads, domain restructuring).

---

## Product Brands

List your organization's product brands. Mark your own brand for exclusion in cross-domain queries.

- **Brand A** — Description of what this brand covers. **YOUR BRAND — exclude in cross-domain filtering.**
- **Brand B** — Description.
- **Brand C** — Description.

## Product Domains

Domains represent functional areas within brands. List the ones most relevant for cross-domain overlap with your work.

- **Domain 1** — Description. Note overlap surface with your domain.
- **Domain 2** — Description.

## Squads

Squads are the teams that build features. Knowing what a squad owns helps evaluate whether a JPD idea's summary indicates functional overlap.

### Brand A Squads
- **Squad Name** — What they own, what they build.

### Brand B Squads
- **Squad Name** — What they own, what they build.

## Product Surface Areas

Surface areas are the specific product capabilities within a squad's scope. Used for fine-grained overlap detection.

- **Surface Area Name** — Description of the capability.
