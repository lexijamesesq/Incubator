...
**Distinguishability pre-screen (before break authoring).** Rivals must differ materially in wedge, buyer, or surface; two variants of one position are one thesis — collapse them before authoring break conditions. **Envelope-entry recipe.** For a constraint-envelope entry, compose rivals by outcome-first gap analysis: score current-state→target gaps, bundle them into positions.
...
2. **If absent — co-draft interactively.** Load `brief-template.md` and `attack-patterns.md` (both bundled). Walk the human through it: they state the strategic claim(s); you help structure them into the required sections and press for falsifiable break conditions. Break conditions are generated, not improvised — per `attack-patterns.md` §C: enumerate each thesis's must-hold conditions (what-would-have-to-be-true), fragility-rank them, derive break conditions from the most fragile, then add the chosen palette attacks (§C selection rule: the two universals + one bespoke). You assist with structure and phrasing; the human owns every strategic claim. If the human declines to co-draft, STOP (a thesis-test with no thesis is not a run — see Stop Rules).
...
4. **Cross-cutting battery (§B)** — run the `attack-patterns.md` §B attacks over the stack-rank; the lenses individually pass what these catch. Two §B members are broken out as their own required operations — the **through-line trace** (this op) and the **evidence-vs-interpretation partition** (op 5); run the remaining §B attacks here (collapse-into-sibling, altitude reframe, time-bounded moat, historical reread, bad-strategy screen, non-diagnosticity flag, info-quality flag) and record any that fire. **Through-line trace:** connect the surviving position back through the evidence to a learning/product surface. A position that only reaches a buyer/procurement surface without a learning/product through-line is procurement-coherent but product-incoherent — flag it.
